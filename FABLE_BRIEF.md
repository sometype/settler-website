...
- [x] **`source` / `source_id` / `url` dropped from the public view** — shipped 2026-07-25 via `sql/006_listings_public_canonical.sql`, which supersedes 005 (006 = 005 + the dedupe alias filter). ⚠️ **005 must never be applied now**; doing so would drop the `canonical_id is null` filter and put duplicate cards back on the site. Verified as anon: `select source from listings_public` → `42703 column does not exist`.
- [x] **One apartment, one card.** `listings_public` also filters `canonical_id is null`; `dedupe_worker.py` matches on phone + deal_type + rooms + area±3m² + same floor + a distance gate. Feed went 899 → 845. Hero counts self-corrected with no front-end change, which finally makes the existing "filtered of agents and duplicates" claim true.
...
- [ ] First-party images: **half done — files are in the bucket, the door is still locked.** All 10,230 photos are now in Cloudflare R2 (`listingpics`, 587 MiB, keys mirror `stored_path` exactly), but **public access is still disabled** and `NEXT_PUBLIC_IMAGE_BASE_URL` is still empty, so the site keeps hotlinking. To finish: enable public access (custom domain `img.settler.ge` — already on Cloudflare nameservers, unlike mepatrone.com — or the rate-limited r2.dev URL), set the env var, add a recurring `rclone copy` timer (the upload was one-time; new photos will not appear on their own), then revoke `listing_images.source_url` from anon. That also removes the proxy's cold-fetch cost (~0.5–1.2s uncached in dev; CDN-cached for a year in production).
